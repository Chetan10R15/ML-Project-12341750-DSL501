...
def create_enhanced_data():
    ...
    raw_data = {
        ...
        'persona': [
            ...
            "['broke up with girlfriend', 'heartbroken', 'cannot stop thinking']",
            ...
            "['overwhelmed', 'wants to be heard', 'does not want advice']",
            ...
            "['grandmother died', 'grieving', 'cannot cope', 'recent loss']",
    ...
    data = []
    for i in range(len(raw_data['dialogue_history'])):
        persona = raw_data['persona'][i]
        if isinstance(persona, str):
            try:
                persona = ast.literal_eval(persona) if persona.startswith('[') else [persona]
        ...
        data.append({
            'dialogue_history': raw_data['dialogue_history'][i],
            'persona': persona,
            'response': raw_data['response'][i],
            'feedback_type': raw_data['feedback_type'][i],
            'strategy': raw_data['strategy'][i]
        })
    
    print(f"✓ Created {len(data)} high-quality training examples")
    print(f"✓ Feedback distribution:")
    fb_counts = pd.Series([d['feedback_type'] for d in data]).value_counts()
    print(fb_counts)
